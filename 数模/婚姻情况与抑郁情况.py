...
def process_marital_depression(file_path):
    # ================== 数据读取 ==================
    try:
    ...
    try:
        # 定义关键列
        marital_cols = ['未婚', '已婚', '离异', '丧偶']
        depression_cols = ['轻度', '中度', '重度']

        # 处理空白和非数字字符
        for col in marital_cols + depression_cols:
            # 转为字符串处理特殊字符
        ...
        df['婚姻状况'] = df.apply(lambda x: get_category(x, marital_cols), axis=1)
        df['抑郁程度'] = df.apply(lambda x: get_category(x, depression_cols), axis=1)

        # 过滤无效数据
        invalid_data = df[df[['婚姻状况', '抑郁程度']].isnull().any(axis=1)]
        df = df.dropna(subset=['婚姻状况', '抑郁程度']).copy()
    ...
    try:
        ...
        for group in df['组别'].unique():
            ...
            # 图表装饰
            plt.title(f'组别 {group} - 婚姻状况与抑郁程度关系\n', fontsize=14, pad=20)
            plt.xlabel('婚姻状况', fontsize=12, labelpad=15)
            ...
            # 调整布局并保存
            plt.tight_layout()
            plt.savefig(f'组别{group}_婚姻抑郁分析.png', dpi=300, bbox_inches='tight')
            plt.close()
...
if __name__ == "__main__":
    ...
    # 执行分析
    process_marital_depression("data.xlsx")  # 替换为实际文件路径